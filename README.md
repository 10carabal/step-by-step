...
We detected that the current version of Capacitor manages native iOS dependencies via Swift Package Manager (SPM) instead of CocoaPods. This is evidenced by the CapApp-SPM folder generated automatically by cap add ios. This approach simplifies the CI pipeline by eliminating an extra installation step and its associated failure surface.

We encountered a package identity conflict in Swift Package Manager between @capacitor/app (a core plugin not actively used in the app's functionality) and @capacitor-firebase/app (required for Remote Config), as both expose a Swift target with the exact same name. We resolved this by removing the unused dependency, prioritizing the Firebase functionality which is an explicit requirement for the test.
